	...
	<tr valign="top">
		...
		<td>
			...
			<form method="get" action="" style="border:1px solid #ccc;padding:3px;background:#efefef">
				<table>
					...
					<tr>
						...
						<input type="button" value="Print" onclick="window.open ('http://localhost/daisy/printReportCGL.php?l=<?=$_GET['l']?>&r=<?=$_GET['r']?>&p=<?=$_GET['p']?>&t1=<?=$_GET['t1']?>&t2=<?=$_GET['t2']?>&no=<?=$_GET['no']?>&s1=<?=$_GET['s1']?>&s2=<?=$_GET['s2']?>','daisy','status=0,menubar=1,scrollbars=1,resizable=1,width=794,height=1123');" />
						<input type="button" value="Export ke excel" onclick="window.open ('ExcelTabelCGL.php?s=<?=$_GET['s']?>&m1=<?=$_GET['m1']?>&t1=<?=$_GET['t1']?>&m2=<?=$_GET['m2']?>&t2=<?=$_GET['t2']?>&ui=<?=$user->user_id?>&r=<?=$user['regional']?>','daisy','status=0,menubar=1,scrollbars=1,resizable=1,width=794,height=1123');" />
					</td>
					</tr>
				</table>
			</form> 			
			<?php
				...
				//echo $SQL1;
				if($_GET['l']!='3') $cgl = $db->getArray($SQL1);
				//$db->debug();
			
			//echo "test"; print_r($cgl)?>
			...
			<table cellpadding="3" cellspacing="0" border="1" style="font-size:8pt">
				...
				<tr class="odd">
					<td>Created</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['UNAPPROVED'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="even">
					<td>Approve</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['APPROVED'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="odd">
					<td>Submitted (Total Klaim)</td> <!--------------------------------------------------------->
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['SUBMITTED'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="even">
					<td>Outstanding</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?php $outs= $summ['SURVEY'][$i]+$summ['PAYMENT'][$i]+$summ['INVOICE'][$i]; if($outs>0) echo $outs ?></td>
					<?php } ?>
				</tr>
				<tr class="odd">
					<td style="padding-left:50px">Survey</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['SURVEY'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="even">
					<td style="padding-left:50px">Payment</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['PAYMENT'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="odd">
					<td style="padding-left:50px">Invoice</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['INVOICE'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="even">
					<td>Settled</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['SETTLED'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="odd">
					<td>Closed Case</td>
					<?php for($i=1;$i<=12;$i++){ ?>
						<td style="text-align:right"><?=$summ['CLOSED'][$i] ?></td>
					<?php } ?>
				</tr>
				<tr class="even">
					<td>Total</td>
